...
import { useAuthApi, useUserApi, getTicketApi } from "~/api";
...
const getCode = () => {
  ...
  authApi
    .smsSend({ mobile: phone.value, event: "mobilelogin" })
    .then((res: any) => {
      console.log(res);
...
const submit = () => {
  ...
  const authApi = useAuthApi();
  authApi
    .mobileLogin({ mobile: phone.value, captcha: code.value })
    .then((res: any) => {
...
const model = computed({
  ...
  set: (value) => {
    accessStore.setLoginExpired(value);
  },
});
...
// const { loginExpired } = storeToRefs(accessStore);
watchEffect(() => {
        ...
        <div class="flex login">
          ...
          <div class="phone-box bg-[url('/images/login/bg.png')]">
            <div class="w-[410px] rounded-lg mx-auto mt-10 p-6">
              <!-- 标题 -->
              <div class="mb-10 text-lg font-bold text-center">
                免密登录注册
              </div>
